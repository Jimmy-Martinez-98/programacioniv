...
var mostrardirecciones = new Vue({
  ...
  methods: {
    ...
    info: function () {
      fetch(
        `private/Modulos/direcciones/procesos.php?proceso=recibirdireccionView&direction=${JSON.stringify(
          this.direction
        )}`
      )
        .then((resp) => resp.json())
...
var editardirecciones = new Vue({
  ...
  methods: {
    ...
    actualizar: function () {
      fetch(
        `private/Modulos/direcciones/procesos.php?proceso=recibirDatos&direction=${JSON.stringify(
          this.modDirec
        )}`
      )
        .then((resp) => resp.json())
...
var nuevadireccion = new Vue({
  ...
  methods: {
    ...
    idlogueo: function () {
      fetch(
        `Private/Modulos/direcciones/procesos.php?proceso=idlogueo&direction=""`
      )
        .then((resp) => resp.json())
        .then((resp) => {
